...
/// `on_showing`, `on_shown`, `on_hiding`, `on_hide`, and `enable_swipe`.
///
/// Each route tracks its visibility state and displays lifecycle event history.
/// The "No Swipe" page demonstrates overriding the router's swipe-back setting.
...
/// A page that disables the swipe-back gesture via `enable_swipe`.
struct NoSwipePage {
    events: Vec<&'static str>,
}
